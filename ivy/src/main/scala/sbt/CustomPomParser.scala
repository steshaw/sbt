...
object CustomPomParser {
  ...
  private[this] def makeCoords(dd: ModuleRevisionId): String = s"${dd.getOrganisation}:${dd.getName}:${dd.getRevision}"
  ...
  private[this] def transformedByThisVersion(md: ModuleDescriptor): Boolean =
    {
      if (md.getModuleRevisionId.getOrganisation == "com.example") System.err.println(s"Checking to see if $md has been transformed.")
